...
import { Download, CheckCircle2, Loader2, FileText, User, Hash, Mail } from 'lucide-react';
import type { Booking, RestoreState } from '@/lib/types';
import { EmailModal } from '@/components/EmailModal';

type Mode = 'guest' | 'refs';

export function StatementPage({ restore, onRestored }: { restore?: RestoreState | null; onRestored?: () => void }) {
  ...
  const handleDownload = async () => {
    ...
    setDone(false);
    try {
      await downloadElementAsPdf('statement-doc', `Receipt-${(selectedGuest || 'guest').replace(/\s+/g, '_')}.pdf`);
      await logDocument('statement', selectedGuest || selectedBookings[0]?.name || 'Guest', hotelDetails.hotel, Array.from(selectedRefs), undefined, user?.email);
      setDone(true);
                  ...
                  {guestBookings.map((b) => {
                    ...
                    return (
                      <tr key={b.bookingNumber} className={checked ? 'bg-blue-50' : 'hover:bg-gray-50'}>
                        ...
                        <td className="px-3 py-2.5 text-gray-600">{b.methodOfPayment}</td>
        ...
        <div className="sticky top-0 space-y-3">
          ...
          {done && (
            <div className="flex items-center gap-2 text-sm text-green-600 font-medium bg-green-50 px-3 py-2 rounded-lg">
              <CheckCircle2 size={15} /> Receipt downloaded successfully
            </div>
          )}
          ...
          {selectedBookings.length > 0 ? (
            ...
            <div className="bg-white rounded-2xl border border-dashed border-gray-300 p-12 text-center">
              ...
              <p className="text-xs text-gray-400 mt-1">The receipt preview will appear here</p>
            </div>
          )}
        </div>
      </div>
      <EmailModal open={emailOpen} onClose={() => setEmailOpen(false)}
        docType="Receipt" guestName={selectedGuest || selectedBookings[0]?.name || 'Guest'}
